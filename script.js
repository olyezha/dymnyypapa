$(document).ready(function () {
    ...
    let delivery = document.getElementById('one')
    let pickUp = document.getElementById('two')
    ...
    conf.onclick = function () {
        ...
        data2.reset();
        var Message;
        Message = "Повне ім'я: " +  orders.fullName + '\n';
        Message += "Номер телефону: " +  orders.number + '\n';
        Message += "Комментар до замовлення: " +  orders.CommentToOrder + '\n';
        if (orders.city) {
            Message += "Місто: " +  orders.city + '\n';
            Message += "Вулиця: " +  orders.street + '\n';
            Message += "Будинок: " +  orders.house + '\n';
        }
        else Message += "Тип замовлення: Самовивіз" + '\n';
        Message += "Оплата: " +  orders.payment + '\n';
        // res();
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "https://api.telegram.org/bot1728401005:AAFSEnOV77mIAkEyhoZSnj4JjiVEtCwh0ZM/sendMessage",
            "method": "POST",
            "headers": {
